...
void App::run() {
    while (!window.shouldClose() && !controller.shouldExit()) {
        ...
        else if (controller.getActiveScreen() == GUIActiveScreen::Save) {
            saveScreen.update(controller);
        }
        ...
        else if (controller.getActiveScreen() == GUIActiveScreen::Save) saveScreen.draw(controller);
